...
class Problem:
    ...
    def calcdist(self,Xpoints):
        ...
        for jj in range(np.size(self.ypar,axis=0)):
            current_X = self.Xall[np.where((self.yall[:,0] == self.ypar[jj,0])&(self.yall[:,1] == self.ypar[jj,1]))]
            norm_curX = standardize(current_X,range=np.vstack((self.lb, self.ub)))/2 + 0.5
            norm_new = (standardize(Xpoints,range=np.vstack((self.lb, self.ub)))/2 + 0.5 - norm_curX)*100
            fileloc ="../innout/tim/Out/dist_nextpoints8_AT_"+str(jj+1)+".csv"
            np.savetxt(fileloc, np.vstack((current_X,norm_new)), delimiter=",",
