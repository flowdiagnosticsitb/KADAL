...
def run_akmcs(krigobj,init_samp,problem,filename,figloc,posdata,negdata):
    ...
    akmcsobj.run(savedatato=filename,logging=False, saveimageto=figloc,plotdatapos=posdata,plotdataneg=negdata)
    elapsed = time.time() - t
...
if __name__ == '__main__':
    ...
    figloc = 'akmcsupdate'
    init_samp = mcpopgen(type='normal', ndim=nvar, n_order=6, n_coeff=1,stddev=1.5)
    ysamp = evaluate(init_samp,problem)
    krigsamp = mcpopgen(type='normal', ndim=nvar, n_order=1, n_coeff=2.5,stddev=1.5)
    ...
    xtotal=run_akmcs(krigobj,init_samp,problem,filename,figloc,pos_MC,neg_MC)
    print(Pfreal)
    ...
    plt.scatter(krigsamp[:, 0],krigsamp[:, 1],c='red',label='Initial Population')
    plt.scatter(updateX[:, 0], updateX[:, 1],s=75,c='black',marker='x', label='Update')
