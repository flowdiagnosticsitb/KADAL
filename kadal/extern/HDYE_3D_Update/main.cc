...
void doscheme(char *schemename, deque<shared_ptr<individual>> & testcase, double r[], vector<shared_ptr<mus>> & pdf){
  double answer;
...
void loadtestcase(char *filename, deque<shared_ptr<individual>> & testcase, double r[], vector<shared_ptr<mus>> & pdf){
// void loadtestcase(char *filename, deque<individual*> & testcase, double r[], vector<mus*> & pdf){
  ...
  for (int i=0;i<n;i++){
    ...
    // shared_ptr<individual> tempvidual;
...
int main(int argc, char *argv[]){
  ...
  int n;
  deque<shared_ptr<individual>> testcase;
  double r[DIMENSIONS];
  ...
  if (argc > 1){
	...
	ofstream test("Time_IRS.txt", ios::app);
