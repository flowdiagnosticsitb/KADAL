...
//EHVI calculation algorithm with time complexity O(n^3).
  ...
  // shared_ptr<thingy> Pstruct; //2D array with information about the shape of the dominated hypervolume
  ...
  try{
    ...
    // Pstruct  = make_shared<thingy>();  // What should this be?
    Pstruct = new thingy[n*n];
    for (int k=0;k<n*n;k++){
      // auto Pstruct[k] = make_shared<thingy>();
      Pstruct[k].slice = 0;
      Pstruct[k].chunk = 0;
