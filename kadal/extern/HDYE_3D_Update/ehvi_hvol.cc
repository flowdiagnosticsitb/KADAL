...
//Returns the previous node in the sort order, or NULL if there is no previous node
  avlnode * answer;
  if (start == NULL){
    return NULL;
  }
  if (start->left){ //return rightmost left child
    ...
    return answer->parent; //return node that start is the leftmost right child of
  }
}
...
//Returns the next node in the sort order, or NULL if there is no next node
  ...
  if (start->right){ //return leftmost right child
    ...
    while (answer->parent != NULL && answer->parent->left != answer)
      answer = answer->parent;
    return answer->parent; //return node that start is the rightmost left child of
  }
}
...
//If this is the first candidate, it can always be inserted.
  ...
  nodecount++;
  //We have inserted the point and must now update totalS and remove dominated points.
  calculateS(temp);
...
//Iterates over points that come after node, stops when it finds one that isn't dominated
//or reaches the end. First nondominated point's horizontal S strip is recalculated.
//Dominated points are deleted.
  ...
  while (previous != NULL && previous->data->y <= node->data->y){
    ...
    point * magic;
    if (previous != NULL)
      magic = previous->data;  //Protection against the magic trick in removenode
...
//Display the contents of the tree.
  ...
  for (int i=0;i<depth;i++)
    cout << "-";
  cout << " X: " << node->data->x << " Y: " << node->data->y << " S: " << node->data->S
...
double hvol3d(deque<shared_ptr<individual>> P, double cl[DIMENSIONS], double fmax[DIMENSIONS]){
// double hvol3d(deque<individual*> P, double cl[DIMENSIONS], double fmax[DIMENSIONS]){
  //Calculate the 3d hypervolume bounded by cl (from below) and fmax (from above).
  ...
  if (P[P.size()-1]->f[0] > cl[0] && P[P.size()-1]->f[1] > cl[1])
    tree.attemptcandidate(min(fmax[0],P[P.size()-1]->f[0]),min(fmax[1],P[P.size()-1]->f[1]));
  oldz = P[P.size()-1]->f[2];
...
double calculateslice(deque<shared_ptr<individual>> P, double r[DIMENSIONS], double fmax[DIMENSIONS], int dimension){
...
//point r.
double calculateS(deque<shared_ptr<individual>> P, double r[]){
